...
class PINN3D(PINN1D):
    def __init__(self, *args, **kwargs):
        ...
        self.transform = tf.keras.layers.Lambda(
            ...
            # lambda x: (self.Tbot - self.Tgrad(x[:, 0:1], x[:, 1:2], x[:, 2:3]) * x[:, 3:4])   
        )
        
    def Tgrad(self, t, x, y, z):
        raise NameError('geothermal gradient function needs to be defined')
...
class PINN1DSolver():
    ...
    def fun_u(self, t):
        raise NameError('uplift function needs to be defined')
        # return self.uplift
    ...
    def save_history(self, file):
        np.savez(file, it_hist=self.it_hist, loss_hist=self.loss_hist,
                 heat_hist=self.heat_hist, bound_hist=self.bound_hist)

    def get_runpath(self):
        runpath = os.path.join(self.savepath, self.runname)
        if not(os.path.exists(runpath)):
            os.mkdir(runpath)

        return runpath
                 
    def callback(self, echofreq=100, savefreq=1000):
        self.iter += 1
        ...
        if (echofreq > 0) and self.iter % echofreq == 0:
            print('{} It {:5d}: loss = {:10.8e}: run time = {:g} seconds'.format(
                  self.runname, self.iter, self.current_loss, (datetime.datetime.now()-self.start_time).seconds)
                 )
            
        if (savefreq > 0) and (self.iter % savefreq == 0):
            modelpath = os.path.join(runpath, str(self.iter))    
            tf.saved_model.save(self.model, modelpath)
...
class PINN3DSolver(PINN1DSolver):    
    ...
    def fun_h(self, t, x0, y0):
        raise NameError('topography function needs to be defined')
